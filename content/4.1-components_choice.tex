...
Toutes les références de prix ont été prises sur DigiKey~\cite{digikey}. Les écarts de prix entre les candidats retenus se sont révélés marginaux à l'échelle de la BOM complète de la carte. Le prix n'a donc jamais constitué un critère décisif, ni été opposé à la priorité donnée à la qualité audio.
...
La diffusion audio vers une enceinte ou un casque Bluetooth repose sur le profil \textit{Advanced Audio Distribution Profile} (A2DP), qui relève du Bluetooth Classic et non du Bluetooth à basse consommation (BLE)~\cite{bluetooth_sig_a2dp}. Un micro-contrôleur ne disposant que d'une radio BLE est donc incapable de transmettre un flux A2DP, quelle que soit sa puissance de calcul. Cette contrainte élimine la grande majorité des micro-contrôleurs sans fil du marché : les familles nRF52 et nRF53 de Nordic Semiconductor~\cite{nordic_nRF52840}, les STM32WB de STMicroelectronics~\cite{stm_STM32WB55} et les variantes récentes de l'ESP32 lui-même (S3, C3, C6)~\cite{espressif_soc_comparison} n'intègrent qu'une radio BLE. L'ESP32 d'origine est l'un des rares composants à la fois disponibles, peu coûteux et bien documentés à combiner le Bluetooth Classic et l'A2DP en mode source, via la pile Bluedroid de son environnement de développement~\cite{ESP32_IDF_guide}.
...
Le PCM5242 est le plus cher des trois candidats (8,06~CHF), seul écart de prix notable du chapitre, mais négligeable au regard du gain de qualité audio.
...
Quatre convertisseurs buck de 1\,A ont été comparés (tableau~\ref{tab:comparo_buck}). Les quatre partagent les fonctions attendues d'un buck moderne, à savoir le mode light-load automatique, le fonctionnement à 100\,\% de rapport cyclique, la protection contre les surintensités et le démarrage progressif (soft-start). Ces fonctions ne les départageant pas, la sélection s'opère sur le rendement, la consommation à vide, le boîtier et le prix. maximal étant identique, la sélection s'opère sur le rendement, la consommation à vide, le boîtier et le prix.
