...
Les quatre composants couvrent le besoin de manière équivalente. Le PI4IOE5V9554 a été retenu parce que son brochage s'accorde particulièrement bien avec l'implantation des composants sur le circuit imprimé, ce qui permet un routage plus simple, son prix et son courant de repos restant par ailleurs modestes et sans incidence à l'échelle de la carte.
...
Le dimensionnement du régulateur 3,3~V suppose de connaître la consommation de chaque composant qu'il alimente. Le plus énergivore étant le micro-contrôleur lors du streaming Bluetooth, une mesure détaillée à l'annexe~\ref{ann:conso_esp32} a été réalisée et donne un courant moyen de 137~mA et un pic de 321~mA, cette dernière valeur étant arrondie à 350~mA pour aligner la marge sur le pire cas spécifié par la datasheet~\cite{ESP32_Datasheet}. Le tableau~\ref{tab:conso_totale} recense l'ensemble des composants du rail 3,3~V, en retenant pour chacun sa valeur maximale afin de dimensionner dans le pire cas.
...
La jauge de batterie estime l'état de charge de la cellule, l'objectif principal étant d'afficher à l'utilisateur le pourcentage de charge restant (\gls{soc}) et l'objectif secondaire de rapporter le plus de grandeurs annexes possible, utiles au suivi de la batterie sans être indispensables. Le choix se joue donc sur la précision de l'estimation et sur la richesse des données fournies. Deux approches existent, la mesure de la seule tension de la cellule, simple mais sensible aux variations de charge, et le comptage de coulomb, qui intègre le courant réellement échangé pour un suivi plus fidèle. Les jauges à résistance de mesure intégrée auraient évité un composant supplémentaire mais ne sont proposées qu'en boîtier \gls{bga}, écarté pour ce projet, de sorte que la recherche s'est orientée vers des jauges à résistance externe (tableau~\ref{tab:comparo_jauge}).
...
Le STC3100 est écarté en premier, ne calculant pas l'état de charge et ne remplissant donc même pas l'objectif principal. Le MAX17048, dépourvu de compteur de coulomb, fonde son estimation sur la seule tension et se dégrade sous charge variable. Le BQ27441-G1 constitue un candidat sérieux mais ne renseigne ni l'autonomie estimée ni le nombre de cycles. Le MAX17260 a été retenu parce qu'il associe un compteur de coulomb à l'algorithme ModelGauge m5, qui combine la mesure de tension et l'intégration du courant et demeure ainsi précis même sous charge variable, parce qu'il est le seul candidat à rapporter l'ensemble des grandeurs utiles, dont l'autonomie estimée particulièrement précieuse pour l'affichage, et parce qu'il présente la plus faible consommation du comparatif avec 30~$\mu$A en mode actif contre 93~$\mu$A pour le BQ27441-G1. Il nécessite une résistance de mesure externe, conséquence acceptée de la contrainte de boîtier.
